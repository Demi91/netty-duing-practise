...
编写代码的逻辑：
1）声明出相关的数据结构，包含业务中的类别枚举
2）区分不同的代码块所处理的业务逻辑   
   handler中  接收了请求路径 进行哪些处理 （通用处理）
      再一步根据类别  调用service服务（个性化处理）
      数据层   使用本地内存 LocalData

   要存储的数据需明确：
   通道列表、通道和用户id的关联关系、在线用户列表、群组列表    
   
   handler - service -  LocalData

3) 明确具体的业务逻辑   
     如用户上线，在线用户列表增加新用户，关联关系增加关联
         拿到各种列表返回    广播通知所有用户

     如用户下线，在线用户列表删除新用户，关联关系去掉关联   

     如新增群组，群组列表中增加，新群组的用户列表自然包含创建人

     如加入群组，群组列表中查找指定群，修改群用户列表，增加新用户

     如私聊消息，通过两个uid，对应两个通道，进行通知

     如群聊消息，在群组列表中找到群，通过群找到用户列表，再通过用户id找到通道，通知所有相关通道  （消息的处理过程相对隔离）


可以扩展的内容：
存储数据到redis、
删除群组、管理群成员、上传文件到群里等等业务功能
增加用户的登录状态、注册信息等
存储聊天信息、进行相应查询
扫码登录的逻辑等等
...
TCP经过三次握手建立连接，四次握手释放连接，使用是可靠的
UDP随时发送数据，可能出现数据丢失，使用是不可靠的


早期的QQ以UDP为主，TCP为辅，主要原因在于成本（TCP使用的网络宽带等更高）
登录、查询、收发消息，主要使用TCP
语音、视频通话时，默认能够接受部分数据丢失，使用UDP
